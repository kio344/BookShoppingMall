...
public class ProductDao {
	...
	public List<ProductRequestDto> gets(int start, int offset) {
		...
		TypedQuery<ProductRequest> query = em.createQuery(
				"SELECT p FROM ProductRequest p WHERE p.progress=:progress order by p.num desc ", ProductRequest.class);
	...
	public List<ProductRequestDto> getSearchProduct(int start, int offset, String searchValue, String searchType) {
		...
		String sql = "SELECT p FROM ProductRequest p WHERE p.progress=:progress AND p." + searchType
				+ " like :searchValue order by p.num desc ";
		TypedQuery<ProductRequest> query = em.createQuery(sql, ProductRequest.class);
	...
	public int getSearchProductCount(String searchValue, String searchType) {
		...
		String sql = "SELECT COUNT(*) FROM ProductRequest p WHERE p.progress='Agree' AND p." + searchType + " like '%"
				+ searchValue + "%'";
	 ...
	 * @return
	 */
	public ProductRequestDto updateProductScore(Long productNum) {

		StoredProcedureQuery query = em.createNamedStoredProcedureQuery("updateProductScore");

		query.setParameter("updateBookNum", productNum);

		ProductRequest result = (ProductRequest) query.getSingleResult();
		
		em.detach(result);
		
		result = em.find(ProductRequest.class, result.getNum());
		
		return ProductRequestDto.toDto(result);
	}
